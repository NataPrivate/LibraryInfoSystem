...
namespace LibraryInfoSystemClient
{
    public partial class InsertForm : Form
    {
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            ...
            else if (Owner != null && Operation == ViewForm.Operation.EnterPersonalCabinet)
            {
                string userId = GetUserId("reader", "full_name", textBoxes[0].Text);
                ((ViewForm)Owner).ClientID = Convert.ToInt32(userId);
                ((ViewForm)Owner).EnterPersonlCabinet();
                Close();
            }
            else
                MessageBox.Show("Запис був успішно доданий.", "Шановний користувач!", MessageBoxButtons.OK,
