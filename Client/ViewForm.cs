...
namespace Client
{
    public partial class ViewForm : Form
    {
        ...
        {
            if (UserLogin != "admin" && UserLogin != "librarian")
            {
                operationsToolStripMenuItem.Enabled = false;
                personalCabinetToolStripMenuItem.Visible = true;
            }
            if (UserLogin == "reader" || UserLogin == "publisher")
                foreach (ToolStripItem item in tablesToolStripMenuItem.DropDownItems)
                    if (item.Text != "book" && item.Text != "author" &&
                        item.Text != "book_author" && item.Text != "publisher")
                        item.Visible = false;
        }
        ...
        private void SetDefaultProperties(string tableName)
        {
            ...
            // set FullRowSelect for reader to be able to book books
            if (tableName == "book" && UserLogin == "reader")
            {
        ...
        private void ProceedDelete()
        {
            ...
            else if (table.TableName.Equals("department") || table.TableName.Equals("hall"))
            {
                for (int i = 0; i < dataGridView.SelectedRows.Count; i++)
        ...
        private void personalCabinetToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (ClientID != -1)
            {
                EnterPersonlCabinet();
            ...
            CurrentOperation = Operation.EnterPersonalCabinet;
            DialogResult regist = MessageBox.Show("Ви вже зареєстровані у нашій системи?", "Шановний користувач!", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (regist == DialogResult.Yes)
                new LogInForm(UserLogin, CurrentOperation).Show(this);
            else
                new InsertForm(GetTable("reader"), CurrentOperation).Show(this);
        }
        public void EnterPersonlCabinet()
        {
            Invoke(new Action(delegate
            {
                new PersonalCabinetForm(this, UserLogin, ClientID).Show(this);
            }));
        }
        ...
        private void btnBook_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Не обрано жодної книги для бронювання.", "Шановний користувач!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            DialogResult confirm = MessageBox.Show("Ви впевнені, що бажаєте забронювати на обрані книги?", "Шановний користувач!", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (confirm == DialogResult.Yes)
            {
                CurrentOperation = Operation.Book;
                if (ClientID != -1)
                {
                    Book();
                    return;
