...
namespace Client
{
    public partial class AuthorizationForm : Form
    {
        ...
        private void buttonLog_Click(object sender, EventArgs e)
        {
            ...
            {
                Hide();
                SendServerConnectionUser(login, password);
                Thread.Sleep(100);
        ...
        private bool VerifyMd5Hash(string login, string password)
        {
            ...
            try
            {
                outputDB = GetMd5HashFromServer(login);
            }
            catch
            {
        ...
        private string GetMd5HashFromServer(string login)
        {
            // Создаем запрос
            KeyValuePair<string, IList<string>> requestData =
                new KeyValuePair<string, IList<string>>("pass", new List<string> { login });
        ...
        private void SendServerConnectionUser(string login, string pass)
        {
            KeyValuePair<string, IList<string>> requestData =
                new KeyValuePair<string, IList<string>>("initHandler", new List<string> { login, pass, "library" });
            var binFormatter = new BinaryFormatter();
            var mStream = new MemoryStream();
            binFormatter.Serialize(mStream, requestData);
            byte[] byteRequest = mStream.ToArray();

            SendRequestToServer(byteRequest, false);
        }
    }
}
